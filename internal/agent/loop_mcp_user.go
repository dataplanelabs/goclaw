...
func (l *Loop) getUserMCPTools(ctx context.Context, userID string) []tools.Tool {
	...
	for _, info := range l.mcpUserCredSrvs {
		srv := info.Server
		oauthActive := mcpbridge.IsOAuthActive(srv.Settings)
		...
		if !oauthActive {
			...
			hasStaticCreds = err == nil && uc != nil && (uc.APIKey != "" || len(uc.Headers) > 0 || len(uc.Env) > 0)
		}
		...
		// Inject server-level API key only for non-OAuth servers. OAuth servers
		// must not inherit static Authorization as a fallback before user consent.
		...
		if hasOAuthCreds {
			token, err := l.mcpOAuthTokenProvider.GetValidToken(ctx, srv.ID, l.tenantID, userID)
			if err != nil || token == "" {
				if err != nil {
					slog.Warn("mcp.oauth_token_unavailable", "server", srv.Name, "user", userID, "tenant", l.tenantID, "error", err)
				}
				continue
			}

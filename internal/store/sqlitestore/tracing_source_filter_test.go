...
func TestBuildTraceWhere_SourceTypeFilter(t *testing.T) {
	...
	cases := []struct {
		...
		{"cron", "session_key LIKE ?", []any{"%:cron:%"}},
		{"group", "session_key LIKE ?", []any{"%:group:%"}},
		{"team", "session_key LIKE ?", []any{"%:team:%"}},
		{"direct", "(session_key LIKE ? AND session_key NOT LIKE ?)", []any{"%:direct:%", "%:ws:%"}},
		{"ws", "(channel = ? OR session_key LIKE ?)", []any{"ws", "%:ws:%"}},
		{"", "", nil},
		{"bogus", "", nil},
	}

...
func (s *SQLiteAgentStore) Create(ctx context.Context, agent *store.AgentData) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		agent.AgentType, agent.IsDefault, agent.Status, agent.BudgetMonthlyCents, agent.WriteOnlyHash,
		now, now, tenantID,
	)
	return err
}
...
func scanAgentRow(row agentRowScanner) (*store.AgentData, error) {
	...
	err := row.Scan(
		...
		&d.AgentType, &d.IsDefault, &d.Status, &d.BudgetMonthlyCents, &d.WriteOnlyHash,

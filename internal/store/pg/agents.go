...
const agentSelectCols = `id, agent_key, display_name, frontmatter, owner_id, provider, model,
		 ...
		 agent_type, is_default, status, budget_monthly_cents, write_only_hash, created_at, updated_at, tenant_id`
...
func (s *PGAgentStore) Create(ctx context.Context, agent *store.AgentData) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		agent.AgentType, agent.IsDefault, agent.Status, agent.BudgetMonthlyCents, agent.WriteOnlyHash, now, now, tenantID,
	)
	if err != nil {
		return err
	}
...
func scanAgentRow(row agentRowScanner) (*store.AgentData, error) {
	...
	err := row.Scan(&d.ID, &d.AgentKey, &d.DisplayName, &frontmatter, &d.OwnerID, &d.Provider, &d.Model,
		...
		&d.AgentType, &d.IsDefault, &d.Status, &d.BudgetMonthlyCents, &d.WriteOnlyHash, &d.CreatedAt, &d.UpdatedAt, &d.TenantID)
	if err != nil {
		return nil, err
	}

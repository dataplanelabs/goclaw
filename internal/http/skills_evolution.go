...
func (h *SkillsHandler) applySkillSuggestionPatch(r *http.Request, skillID uuid.UUID, sg *store.SkillImprovementSuggestion) (*store.SkillImprovementSuggestion, error) {
	...
	priorSource := ""
	if info, ok := h.skills.GetSkillByID(r.Context(), skillID); ok {
		priorSource = info.Source
	}
	...
	// Re-stamp source to 'evolution' so the evolved skill is no longer
	// owned by gcplane and the upload overwrite gate engages on a future
	// reconcile. This makes the git<->runtime divergence visible/auditable
	// instead of silent.
	...
	if priorSource == "gcplane" {
		slog.Warn("skill.evolution.gcplane_restamp",
			"skill_id", skillID.String(),
			"slug", slug,
			"old_version", oldVersion,
			"new_version", newVersion,
			"prior_source", priorSource,
			"note", "evolved a gcplane-managed skill; source re-stamped to 'evolution'. A future gcplane reconcile triggered by a goclaw-config edit to this skill will overwrite the evolved version")
	}

...
// SetEvolutionStore wires the skill self-evolution and activity stores.
...
func (h *SkillsHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	// Skill self-evolution reads (viewer+; admin-gated payload sanitization applied in handlers)
	...
	// Skill self-evolution mutations (tenant admin only; agents cannot self-apply)
...
// tenantAdminMiddleware requires admin role AND (master scope OR tenant-admin
// membership). Self-evolution mutations are gated here so agents cannot
// self-apply — only an authenticated tenant admin can.

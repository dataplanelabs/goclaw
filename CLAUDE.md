...
- **JPEG XL inbound:** `.jxl` files arrive from Zalo HD photos (and elsewhere) — LLM providers (Anthropic / OpenAI / Gemini) reject `image/jxl`. Handled in two layers: (1) `internal/agent/media_sanitize.go` blank-imports `github.com/gen2brain/jpegxl` (wazero-WASM libjxl, CGo-free), which self-registers a decoder for both raw codestream (`\xff\x0a`) and ISOBMFF container (`????JXL`) magic bytes; `SanitizeImage` then decodes JXL transparently and re-encodes as JPEG q=85. (2) `internal/channels/zalo/personal/handlers.go` `pickInboundImageURL` prefers non-`.jxl` URLs from `attachMediaURLFields` (`normalUrl > oriUrl > href > hdUrl > thumbUrl > thumb`) to skip the 150–400 ms WASM decode when the CDN offers JPEG. The tool-layer `read_image` (`internal/tools/read_image.go::loadImageFromPath`) re-encodes JXL via `imaging.Open` + `jpeg.Encode` so providers see `image/jpeg`. MIME plumbing: `.jxl` → `image/jxl` in `mime_detect.go::DetectMIMEType`, `media.go::inferImageMime`, `store.go::ExtFromMime`.
- **Image gen with reference photos:** `create_image` accepts `reference_image_ids: []string` (media IDs from `<media:image id="…">` tags in the current turn). Tool-layer caps: **4 refs total** (the binding cap today), 5MB per image, 20MB aggregate. Per-provider wire shape: Gemini `inline_data` parts on `:generateContent`; OpenRouter `image_url` parts on `/chat/completions` with `modalities:["image","text"]`; MiniMax `subject_reference:[{type:"character",image_file}]` on `/image_generation` (further capped at 1, character only); OpenAI routes refs-mode to `/v1/images/edits` multipart with `image[]=@…` (provider native cap 16 but tool layer enforces 4, `input_fidelity=high` only for `gpt-image-1*` excl. `mini`, `response_format` omitted for `gpt-image-*` since it 400s and b64 is default). Codex/Responses-API native path DOES support edits via `input_image` parts (codex_native_image.go). DashScope and BytePlus do NOT support refs yet (Phase 04 deferred) — `slog.Warn("create_image: reference images dropped …")` fires on those branches when refs supplied. NO automatic model upgrade. Refs-capability is provider-self-reported via `providers.CapabilitiesAware.Capabilities().ImageRefs` (single source of truth — no name-based string switches in `filterChainForRefs`); operator-configured chain order from UI is preserved. SVG/GIF rejected at resolve time. Ctx plumbing: `tools.WithMediaImageRefs(ctx, refs)` populated in `loop_input_media.go`; `tools.MediaImageRefsFromCtx(ctx)` consumed in `create_image.go::Execute()`.
...
- **Channel-specific tools:** Channel-bound actions (e.g. Zalo Personal polls + reactions) register as flat snake_case tools prefixed with the channel type (`zalo_personal_create_poll`, `zalo_personal_react`, etc.). Each tool checks `ToolChannelTypeFromCtx(ctx) == channels.TypeXxx` first, then resolves the channel via an injected `*ActionFn` setter (DI pattern matching `ChannelSenderAware` / `GroupMemberListerAware`). Canonical reference: `internal/tools/zalo_personal_*.go` + `internal/channels/zalo/personal/actions.go` (with `var _ tools.ZaloPersonalAction = (*Channel)(nil)` as a compile-time interface guard).
- **Zalo Personal inbound reactions:** Listener watches WS `cmd=612` (real-time), `cmd=610/611` (historical replay on reconnect). Events flow through `Listener.Reactions()` → `Channel.onReactionEvent` → 30s coalescer (latest-wins per `thread|user|msg`) → `emitCoalescedReaction` → `HandleMessage` with `metadata["zalo_event"]="reaction_added"` and `synthetic="true"`. Self-reactions dropped by default (opt-in via `listen_self_reactions: true`); historic frames always dropped to avoid replay on reconnect. Kill switches: `disable_polls`, `disable_reactions`.
- **Reply-to plumbing:** `metadata["reply_to_message_id"]` is the cross-channel reply key. Stamped on group inbound (telegram, pancake comments) by default; opted-in per channel for DMs via `channels.DMQuoteChannel` (`zalo_oa`, `zalo_personal`). Channel `Send` translates the key to platform-specific payload — Telegram `ReplyParameters`, Zalo OA `message.quote_message_id`, Pancake `comment_id`, Zalo Personal routes to `/api/message/quote` or `/api/group/quote` with `qmsg*` params. First chunk only; media sends drop quote. Zalo OA silently retries without quote on `FamilyPayload` errors; Zalo Personal does the same on `ErrQuoteRejected` (any non-zero error_code from `/quote`). Zalo Personal additionally carries the full quote object via `metadata["reply_to_quote_payload"]` (JSON-serialized `protocol.TQuote`) — needed because the `/quote` endpoint requires the original text + attach metadata, not just the ID. Other channels ignore `reply_to_quote_payload`. The gateway helper `buildOutboundReplyMeta` preserves a channel-stamped `reply_to_message_id` rather than overwriting it with the current message's ID — required so zalo-personal can stamp `TQuote.GlobalMsgID` (the QUOTED message) rather than the current inbound message.
...
- **Standby mode:** Declarative per-channel + per-thread schedule on `channel_instances.silence_schedule` (JSONB) + `channel_thread_schedules`. Pipeline gate at iteration entry (`internal/pipeline/standby_gate.go`) skips Think/Tool/Observe via `AbortRun` but finalize still writes working + episodic memory. Resolver in `internal/channels/schedule/` (composite-key `(tenantID, channelName, threadKey)` lookup, 60s TTL + push-reload via `ScheduleRegistry`). Self-pause via `enter_standby` agent tool. See `docs/standby-mode.md`.
- **Team-reply capture + eval (Zalo OA):** Per-channel polling worker (`internal/channels/zalo/oa/poll_worker.go`, 60s tick) hits `/onbehalf/conversation` when `config.capture_team_replies=true`; OA-side messages persisted to `sessions.messages` JSONB with `metadata.source="team"` + insert into `team_reply_evaluations` (PG 000074 / SQLite 44) + emit `team.reply.observed`. `JudgeWorker` (`internal/consolidation/judge_worker.go`) subscribes and invokes a per-tenant judge agent via `agent.Router.Get(...).Run(...)` (NOT delegate tool — pipeline-exclusive) to produce diff score + hypothesized bot reply; per-tenant rate limit (10/min, 5 burst) gates concurrency. Web UI "Team Analytics" tab shows distribution + recent captures + JSONL export for fine-tuning. Polling chosen over webhook after research found `oa_send_*` undocumented + empirically NO-GO. See `docs/team-reply-evaluation.md`.
...
- **No real data in tests.** Never use real person names, real chat/message content, real user IDs, group/thread IDs, phone numbers, or other PII in test fixtures. Use clearly-synthetic placeholders (e.g. `Writer One`, `group:testchan:900000000000000001`, sender `100000000000000001`). Applies to Go AND web (`ui/`) tests. Copying a real trace/conversation into a test is a leak — anonymize it.

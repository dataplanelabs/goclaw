...
- **Image gen with reference photos:** `create_image` accepts `reference_image_ids: []string` (media IDs from `<media:image id="…">` tags in the current turn). Tool-layer caps: **4 refs total** (the binding cap today), 5MB per image, 20MB aggregate. Per-provider wire shape: Gemini `inline_data` parts on `:generateContent`; OpenRouter `image_url` parts on `/chat/completions` with `modalities:["image","text"]`; MiniMax `subject_reference:[{type:"character",image_file}]` on `/image_generation` (further capped at 1, character only); OpenAI routes refs-mode to `/v1/images/edits` multipart with `image[]=@…` (provider native cap 16 but tool layer enforces 4, `input_fidelity=high` only for `gpt-image-1*` excl. `mini`, `response_format` omitted for `gpt-image-*` since it 400s and b64 is default). Codex/Responses-API native path does NOT support edits — chain falls through with a refs-specific error message. DashScope and BytePlus do NOT support refs yet (Phase 04 deferred) — `slog.Warn("create_image: reference images dropped …")` fires on those branches when refs supplied. NO automatic model upgrade. SVG/GIF rejected at resolve time. Ctx plumbing: `tools.WithMediaImageRefs(ctx, refs)` populated in `loop_input_media.go`; `tools.MediaImageRefsFromCtx(ctx)` consumed in `create_image.go::Execute()`.

...
- **Testing privacy:** Do not use real customer, staff, sender, company, phone, tax ID, address, email, or trace-derived production names in tests, fixtures, dry runs, demo prompts, filenames, screenshots, examples, or issue reproductions. Use clearly synthetic placeholders such as `Customer A`, `Example Company`, `Nguyen Van A`, `0123456789`, and `demo@example.com`. Real names are allowed only when directly processing the real user request where that identity is required.
